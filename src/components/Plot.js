    ...
    render() {
        ...
        return (
            <div className='container plot'>
                <h2>
                    <i className="fa fa-area-chart" aria-hidden="true"></i>Plot</h2>
                <Form
                    onSubmit=
                    {(values) => { this.props.updatePlot(values); }}
                    defaultValues={{
                    interp: 0,
                    type: 1,
                    r: 14,
                    ...
                    {formApi => (
                        <form onSubmit={formApi.submitForm}>
                            <div className='section properties'>
                                <h3>
                                    <i className="fa fa-sliders" aria-hidden="true"></i>Plot properites</h3>
                                <div>
                                    <label htmlFor="type">Select type of charts:</label>
                                    <Select field='type' options={plotOptions} className='big'/>
                                </div>
                                {this.renderSecondSelect(formApi.getFormState().values.type)}
                                <div>
                                    <label htmlFor="interp">Select interpolation level:</label>
                                    <Select field='interp' options={interpOptions}/>
                                </div>
                                <div>
                            ...
                            <div className='section buttons'>
                                <button className='action' type='submit'>
                                    ...
                                    Draw Plot</button>
                                <button className='action' type='button' onClick={this.saveChart}>
                                    <i className="fa fa-floppy-o" aria-hidden="true"></i>Capture plot</button>
                            </div>
                        </form>
                    )}
                </Form>
...
function mapStateToProps(state) {
    return ({plot: state.plot, data: state.data});
}
